    ...
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="text-center sm:text-left text-xl">
          Wheel of Life {currentDate} {comparisonDate ? `vs ${comparisonDate}` : ""}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="flex flex-col sm:flex-row justify-between items-center mb-4">
          <p className="text-md font-semibold">Current Total: {currentTotalScore.toFixed(1)}</p>
          {comparisonTotalScore !== undefined && (
            <p className="text-md font-semibold">Previous Total: {comparisonTotalScore.toFixed(1)}</p>
          )}
        </div>
